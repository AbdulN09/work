...
class ConversationSummaryPagination(PageNumberPagination):
    page_size = 10
...
class ConversationSummaryListView(generics.ListAPIView):
    ...
    pagination_class = ConversationSummaryPagination
...
class FileUploadView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = self.get_serializer(data={'file': file, 'original_name': file.name})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class FileUploadView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if UploadedFile.objects.filter(file__hash=file_hash).exists():
            return Response({'detail': 'File already uploaded'}, status=status.HTTP_400_BAD_REQUEST)

        # Save file
        serializer = self.get_serializer(data={'file': file, 'original_name': file.name, 'hash': file_hash})
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
